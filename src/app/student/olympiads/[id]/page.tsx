        ...
        {current && (
          <div className="flex-1 bg-white rounded-2xl border border-[#E2E8F0] p-6">
            ...
            <div className="flex justify-between mt-6 pt-4 border-t border-[#E2E8F0]">
              <button onClick={() => setCurrentIdx(Math.max(0, currentIdx - 1))} disabled={currentIdx === 0} className="text-sm text-[#64748B] hover:text-[#0F172A] disabled:opacity-30">← Алдыңғы</button>
              <span className="text-sm text-[#94A3B8]">{currentIdx + 1} / {problems.length}</span>
              <button onClick={() => setCurrentIdx(Math.min(problems.length - 1, currentIdx + 1))} disabled={currentIdx === problems.length - 1} className="text-sm text-[#64748B] hover:text-[#0F172A] disabled:opacity-30">Келесі →</button>
            </div>
          </div>
        )}
      </div>
